...
def PerformMapping(first_group: List[Dict],
    ...
    print(f"{Fore.WHITE}  • Provider: {Config.provider}")
    ...
    if not api_key:
        print(f"{Fore.RED}✗ Error: {provider_name} API key not found")
        print(f"{Fore.WHITE}Please set your {provider_name} API key")
        return None

    try:
        print(f"{Fore.GREEN}✓ {provider_name} client initialized")
...
Return JSON object with 'mappings' array containing all mappings. Use threshold: {Config.threshold}"""
        ...
        print(f"{Fore.WHITE}Provider: {provider_name}")
        print(f"{Fore.WHITE}Model: {Config.model}")
        print(f"{Fore.WHITE}Max Tokens: {Config.max_tokens}")
        print(f"{Fore.WHITE}Temperature: {Config.temperature}")
        print(f"{Fore.WHITE}Top P: {Config.top_p}")
        ...
        if finish_reason == "length":
            print(f"{Fore.YELLOW}⚠ Response was truncated (max_tokens reached)")
...
def parse_optimized_response(response_text: str, is_compact: bool, verbose: bool) -> Optional[List[Dict]]:
    ...
    if code_block_match:
        ...
        if verbose:
            print(f"{Fore.CYAN}Extracted content from markdown code block")
            ...
            if start_idx != -1:
                ...
                if end_idx > start_idx:
                    try:
                        ...
                        print(f"{Fore.GREEN}✓ Extracted mappings using bracket matching")
            ...
            if array_match:
                ...
                if end_idx > 0:
                    try:
                        ...
                        print(f"{Fore.GREEN}✓ Extracted mappings array directly")
                    except json.JSONDecodeError:
                        pass

    # Step 5: If still no results, try to repair truncated JSON
    if mapping_results is None:
        print(f"{Fore.YELLOW}Attempting to repair truncated JSON...")

        # Show end of response to diagnose truncation
        print(f"{Fore.CYAN}Response ending (last 300 chars):")
        print(f"{Fore.WHITE}...{cleaned_text[-300:]}")
        ...
        if open_braces > 0 or open_brackets > 0:
            print(f"{Fore.YELLOW}Found unclosed: {open_braces} braces, {open_brackets} brackets")
            ...
            if last_complete_match:
                ...
                try:
                    ...
                    if isinstance(parsed_json, dict) and "mappings" in parsed_json:
                        mapping_results = parsed_json["mappings"]
                        print(f"{Fore.GREEN}✓ Repaired truncated JSON, recovered {len(mapping_results)} mappings")
                    elif isinstance(parsed_json, list):
                        mapping_results = parsed_json
                        print(f"{Fore.GREEN}✓ Repaired truncated array, recovered {len(mapping_results)} mappings")
                except json.JSONDecodeError:
                    ...
                    try:
                        ...
                        if isinstance(parsed_json, dict) and "mappings" in parsed_json:
                            mapping_results = parsed_json["mappings"]
                            print(f"{Fore.GREEN}✓ Simple repair succeeded, recovered {len(mapping_results)} mappings")
                    except json.JSONDecodeError:
                        pass
